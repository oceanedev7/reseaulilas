...
<a href="{{route ('volunteer-content')}}" class="hover:underline relative top-8 left-8"> 
    <i class="fa-solid fa-arrow-left"></i>
    Revenir à la page d'édition du texte 'Devenir bénévole'
</a>
...
<div class="flex flex-col items-center p-12 space-y-6"> 
    ...
    <form method="POST" action="{{route ('volunteer-content-update', $volunteer->id)}}">
        @csrf

        <div class="mb-4">
            <label class="block text-sm font-semibold  mb-2">Description :</label>
            <input value="{{ $volunteer->description }}" name="description" class="w-full px-3 py-2 border border-black rounded-md shadow-sm focus:outline-none focus:ring-jaune focus:border-jaune">
        </div>
